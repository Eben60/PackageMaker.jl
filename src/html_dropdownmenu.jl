...
function make_dd_menu(parentname, fieldname, options)
    labels = make_dd_label(parentname, fieldname, options)
    ...
    <input size="30" id="$(inputid)" name="$(fieldname)" class="menu_target" value="$(default_opt)" onchange="alert('value changed');oncng(this)" type="text"> 
    <span class="activate_menu" id="$(activatemenuid)">Show licenses</span>  <br>
...
function make_dd_label(parentname, fieldname, options)
    labels = String[]
    idtempl = parentname * "_" * fieldname

    for (i, o ) in pairs(options)
        id = "$(idtempl)_$i"
        nm = id
        value = o
        label =
